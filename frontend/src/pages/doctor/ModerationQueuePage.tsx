    ...
    <div className="max-w-5xl mx-auto space-y-6 px-4 py-4">
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-center justify-between">
        ...
        <button
          ...
          className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-md text-xs font-semibold cursor-pointer"
        >
          ← Back to Triage
        </button>
      </div>
      ...
      {actionMsg && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-3 rounded-md text-center text-xs font-semibold">
          {actionMsg}
        </div>
      )}

      {loading ? (
        <div className="bg-white p-12 rounded-lg text-center text-gray-400 text-sm">
          Loading moderation queue...
        </div>
      ) : queue.length === 0 ? (
        <div className="bg-white p-12 rounded-lg text-center text-emerald-700 font-medium text-sm border border-emerald-100">
          ✓ Moderation queue is clean. No community posts pending review.
        </div>
      ) : (
        <div className="space-y-4">
          {queue.map((post) => (
            <div key={post.id} className="bg-white p-6 rounded-lg shadow-sm border-2 border-amber-200 space-y-4">
              ...
              <div className="bg-amber-50/50 border border-amber-100 p-4 rounded-md text-sm text-gray-800 leading-relaxed font-sans">
                {post.content}
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded-md text-xs font-bold transition cursor-pointer border border-red-200"
                >
                  Reject & Suppress Post
                </button>
                <button
                  onClick={() => handleAction(post.id, 'approve')}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md text-xs font-bold transition cursor-pointer shadow"
                >
